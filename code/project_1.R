...
fviz_eig(image_pca,addlabels=TRUE,main = " the percentage of variances explained by each principal component")
fviz_pca_var(image_pca,  title ="Graph of variables: Positive correlated variables point to the same side of the plot") 
fviz_cos2(image_pca,choice="var",axes=1:2,title="Quality for representations of the variables")
...
fviz_pca_ind(image_pca, geom=c("point"),
             ...
             # label=wine.class,
             repel = TRUE, title = "PCA Graph of individuals.")
fviz_pca_ind(image_pca,axes = c(1, 2),label="none",
             ...
             mean.point=F,  title = " Graph of individuals.")
head(image_pca)
summary(data_1)
...
fviz_eig(image_pca_1,addlabels=TRUE,main = " the percentage of variances explained by each principal component")
fviz_pca_var(image_pca_1) 
fviz_cos2(image_pca_1,choice="var",axes=1:2,title="Quality for representations of the variables")
...
fviz_pca_ind(image_pca_1, geom=c("point"),
             ...
             #col.ind="contrib",  gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             # label=wine.class,
             repel = TRUE,title = "PCA Graph for individuals.")
...
fviz_pca_var(dem_pca_1,title = "PCA Graph for variables (dem)") 
fviz_cos2(dem_pca_1,choice="var",axes=1:2,title = "Quality for representations of the variables (dem)")
...
fviz_pca_ind(dem_pca_1, geom=c("point"),
             ...
             #col.ind="contrib",  gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             # label=wine.class,
             repel = TRUE,title = "PCA Graph for individuals (dem)" )
summary(as.factor(label_dem))

fviz_eig(gop_pca_1,addlabels=TRUE)
fviz_pca_var(gop_pca_1,title="PCA Graph for variables (gop)") 
fviz_cos2(gop_pca_1,choice="var",axes=1:2,title = "Quality for representations of the variables (gop)")
...
fviz_pca_ind(gop_pca_1, geom=c("point"),
             ...
             #col.ind="contrib",  gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             # label=wine.class,
             repel = TRUE,title = "PCA Graph for individuals (gop)")
...
#kernel=linear
model_svm_l=svm(label_train~.,data=data_2_train[,-which(names(data_2_train)%in%"VCF0734")],type='C',kernel='linear')
pre_svm_l=predict(model_svm_l,newdata=data_2_test[,-which(names(data_2_train)%in%"VCF0734")])
table(data_2_test$VCF0734,pre_svm_l,dnn=c("real","predicted"))
###ROC plot
svm_roc_l <- roc(data_2_test$VCF0734,as.numeric(pre_svm_l))
plot(svm_roc_l, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),grid.col=c("green", "red"), max.auc.polygon=TRUE,auc.polygon.col="skyblue", print.thres=TRUE,main='SVM ROC plot kernel = linear')


#kernel=polynomial
model_svm_p=svm(label_train~.,data=data_2_train[,-which(names(data_2_train)%in%"VCF0734")],type='C',kernel='polynomial')
pre_svm_p=predict(model_svm_p,newdata=data_2_test[,-which(names(data_2_train)%in%"VCF0734")])
table(data_2_test$VCF0734,pre_svm_p,dnn=c("real","predicted"))
###ROC plot
svm_roc_p <- roc(data_2_test$VCF0734,as.numeric(pre_svm_p))
plot(svm_roc_p, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),grid.col=c("green", "red"), max.auc.polygon=TRUE,auc.polygon.col="skyblue", print.thres=TRUE,main='SVM ROC plot kernel = polynomial')

#kernel=sigmoid
model_svm_s=svm(label_train~.,data=data_2_train[,-which(names(data_2_train)%in%"VCF0734")],type='C',kernel='sigmoid')
pre_svm_s=predict(model_svm_s,newdata=data_2_test[,-which(names(data_2_train)%in%"VCF0734")])
table(data_2_test$VCF0734,pre_svm_s,dnn=c("real","predicted"))
###ROC plot
svm_roc_s <- roc(data_2_test$VCF0734,as.numeric(pre_svm_s))
plot(svm_roc_s, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),grid.col=c("green", "red"), max.auc.polygon=TRUE,auc.polygon.col="skyblue", print.thres=TRUE,main='SVM ROC plot kernel = sigmoid')
...
fviz_eig(switch_pca_1,addlabels=TRUE, title="the percentage of variances explained by each principal component")
fviz_pca_var(switch_pca_1,title="PCA Graph for variables") 
fviz_cos2(switch_pca_1,choice="var",axes=1:2, title="Quality for representations of the variables")
...
fviz_pca_ind(switch_pca_1,axes = c(1, 2),label="none",
             ...
             mean.point=F,title="PCA Graph for individuals")




switch_pca_2=PCA(data_2[-which(data_2$VCF0734==1|data_2$VCF0734==9),-which(names(data_2)%in%"VCF0734")],scale.unit=1,ncp=5,graph=T)
fviz_eig(switch_pca_2,addlabels=TRUE, title="the percentage of variances explained by each principal component")
fviz_pca_var(switch_pca_2,title="PCA Graph for variables") 
fviz_cos2(switch_pca_2,choice="var",axes=1:2,title="Quality for representations of the variables")
